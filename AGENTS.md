...
- Automatic context has two bounds, not one. The Flutter host collects the static attributes (`platform`, `os_version`, `app_version`, `app_build`, `locale`, `timezone`) from the platform channels concurrently, bounded by an internal per-provider ceiling, fail-closed per field. On a cold first launch those channels warm up together and their tail reaches well past a second, so the ceiling is deliberately generous. Collection overlaps native initialization and any part of the first poll for which it is still pending, and both metadata and readiness must settle before `initialize` returns, so an immediate read after `initialize` sees the attributes that arrived within the ceiling. The overlap means the metadata ceiling rarely adds to a cold start dominated by real provider latency, but it does not reduce startup to a clean `max` of the two bounds, because the first poll cannot begin until the native handle exists. The ceiling is an internal implementation detail, not a public knob, and is separate from the caller's `startup_timeout` (which bounds only the readiness wait). A future release may unify the two into one monotonic initialization deadline; treat that as a deliberate public-API decision. The developer contract is one asynchronous `initialize`, no manual plugin warm-up, and reliable immediate reads once it completes.
