...
- Build the Android `.so` files with `cargo ndk` and copy them into `sdks/android/src/main/jniLibs` for `arm64-v8a`, `armeabi-v7a`, `x86`, and `x86_64`. These libraries are gitignored, and `scripts/build/source-linked-android-demo.sh` links them without rebuilding, so a stale local copy surfaces as undefined `uniffi_*` symbols at link time after an FFI surface change. Rebuild them (same command) before the native build, the same way the React Native jniLibs are refreshed above.
